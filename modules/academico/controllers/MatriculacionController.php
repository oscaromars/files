...
class MatriculacionController extends \app\components\CController {
    ...
    public function actionIndex() {
            ...
            if (count($resultIdPlanificacionEstudiante) > 0) {
                ...
                if (count($resultRegistroOnline) > 0) {
                    ...
                    $becado=1;   //per_id pla_id , pes_id
                     if ($isscholar != NULL ) {
                     // if (count($cholarship) > 0) { }
                      $becas_model = new PlanificacionEstudiante();
                     $materias=$becas_model->getcurrentSubjects($pes_id);  
                     
                     
                   //  print_r($dataPlanificacion[0]['Subject']);
              //print_r($dataPlanificacion[0]['Code']);
            //  print_r($dataPlanificacion[0]['Cost']);
             //  print_r($dataPlanificacion[0]['Credit']);
               ...
               $registro_online_model = new RegistroOnline();
                $registro_online_model->per_id = $per_id;
                    ...
                    $registro_online_model->ron_modalidad = $data_student['mod_nombre'];
                    $registro_online_model->ron_carrera = $data_student['pes_carrera'];
              ...
              foreach ($dataPlanificacion as $materia) {
                
                
                    $registro_online_item_model = new RegistroOnlineItem();
                    $registro_online_item_model->ron_id = $ron_id;
                    $registro_online_item_model->roi_materia_cod = $dataPlanificacion[$cont]['Code'];
                    $registro_online_item_model->roi_creditos = $dataPlanificacion[$cont]['Credit'];
                    $registro_online_item_model->roi_costo = $dataPlanificacion[$cont]['CostSubject'];
                    
                    $registro_online_item_model->roi_materia_nombre = $dataPlanificacion[$cont]['Subject'];
                    $registro_online_item_model->roi_estado = "1";
                    $registro_online_item_model->roi_estado_logico = "1";
                    
                    
                     if ($registro_online_item_model->save()) {
                        $roi_id = $registro_online_item_model->getPrimaryKey();
                    }else{
                        throw new Exception('Error en Registro Online.');
                    }
     ...
     public function actionIndexfunda($per_id) {
        ...
        if (count($result_process) > 0) {
            ...
            if (count($resultIdPlanificacionEstudiante) > 0) {
                ...
                $resultRegistroOnline = $matriculacion_model->checkPlanificacionEstudianteRegisterConfiguracion($per_id, $pes_id, $pla_id);
                if (count($resultRegistroOnline) > 0) {
                    //Cuando existe un registro en registro_onlinevvv
                   //return $this->redirect(yii\helpers\Url::toRoute(['registro', 'per_id' => $per_id]));
                  // $url = yii\helpers\Url::toRoute(['registro', 'cat' => 'all']);
     ...
     public function actionFundacion() {
        $per_id = @Yii::$app->session->get("PB_perid");
        ...
        //$asignaturas_model = new AsignaturasPorPeriodo();
        $asignaturas_model = new AsignacionBecasEstudiante();
       ...
       if ($data['PBgetFilter']) {
             ...
             $model = $asignaturas_model->consultarPorBecarEstudiantes($arrSearch);
            //$model = $asignaturas_model->consultarAsignaturaxPeriodo();
             return $this->render('newfund-grid', [
                       "model" => $model,
             ]);
        } 
        else {
             //$model = $asignaturas_model->consultarAsignaturas();
             //$model = $asignaturas_model->getEstudiantesBecasPorPeriodoAcademico2($arr_periodo[0]['id']);
            $arrSearch["periodo"] = $arr_periodo[0]['id'];
             $model = $asignaturas_model->consultarPorBecarEstudiantes($arrSearch);
        }  
        // $model = $asignaturas_model->getEstudiantesBecasPorPeriodoAcademico2(1);
     ...
     * @return
     */
    public function actionRegistro($per_id=null) { // pantalla para que el estudiante seleccione las materias a registrarse
       ...
       if ($userperid != $per_id) {$noAdd=2; } else {$noAdd=1; } 
        ...
        if (Yii::$app->request->isAjax) {
            ...
            if (isset($data["pes_id"])) {
                ...
                $modelEstudiante = Estudiante::findOne(['per_id' => $per_id, 'est_estado' => '1', 'est_estado_logico' => '1']);                
                $data_estudio    = Matriculacion::getEstudioAcademicoByEstudiante($per_id);
                 if ($modelEstudiante == NULL){ $per_id = $data["per_id"]; 
                  $data_estudio    = Matriculacion::getEstudioAcademicoByEstudiante($per_id);
                 } 
                \app\models\Utilities::putMessageLogFile('>>>>PER 2:'.$data["per_id"]);
                \app\models\Utilities::putMessageLogFile('>>>>PER 3:'.$per_id);    
                ...
                if (isset($data['registerSubject']) && $data['registerSubject'] == 1){
                    ...
                    if(!$model_rpm){
                        $modReg = RegistroAdicionalMaterias::findOne(['per_id' => $per_id, 'ron_id' => $ron_id, 'rama_estado' => '1', 'rama_estado_logico' => '1']);
                         \app\models\Utilities::putMessageLogFile('>>>>RON :'.$ron_id);
                    ...
                    $registro_online_model->per_id = $per_id;
                    $registro_online_model->pes_id = $pes_id;
                    $registro_online_model->ron_num_orden = 8; //Ya no se usa pero no permite null, por eso tiene valor de 0
                    $registro_online_model->ron_anio = date("Y");
             ...
             foreach ($materias as $materia) {
                    ...
                    $registro_online_item_model->roi_creditos = $creditos[$cont];
                    $registro_online_item_model->roi_costo = $costoPrograma->pccr_costo_credito * $creditos[$cont];
                    //$registro_online_item_model->roi_costo = 160 * $creditos[$cont];
                    $registro_online_item_model->roi_materia_nombre = $materia;
                    $registro_online_item_model->roi_estado = "1";
                    $registro_online_item_model->roi_estado_logico = "1";
            ...
            if (count($resultIdPlanificacionEstudiante) > 0) {
                ...
                if ($data_student) {
                    ...
                    //Send email
                        ...
                        //ini-gap 15/04/2021 
                        //variables añadida para la impresion del logo en el pdf
                        ...
                        $base = Yii::$app->basePath;
                        $lang = Yii::$app->language;
                        $body = Utilities::getMailMessage("registrobeca", array("[[user]]" => $modelPersona->per_pri_nombre . " " . $modelPersona->per_pri_apellido, "[[periodo]]" => $data_student["pla_periodo_academico"], "[[modalidad]]" => $data_student["mod_nombre"]), Yii::$app->language, Yii::$app->basePath . "/modules/academico");
                        $titulo_mensaje = Academico::t('matriculacion',"Online Enrollment Record");
    ...
    public function actionListold() { // listar estudiantes por aprobar matriculacion
    ...
    public function actionlist3() {
        ...
        if ($data['PBgetFilter']) {
            ...
            $arrSearch["estado_acepta"] = $data['estado_acepta'];
            //$arrSearch["estado_correo"] = $data['estado_correo'];
            $arrSearch["f_iniofer"] = $data['f_iniofer'];
            $arrSearch["f_finofer"] = $data['f_finofer'];
        ...
        return $this->render('index', [
                    ...
                    'arr_estadoacepta' => $this->estados_aceptacion(),
                    'arr_estadocorreo' => $this->estados_envcorreo(),
        ]);
    }
        ...
        if (Yii::$app->request->isAjax) {
            ...
            try {
                ...
                $registro_pago_matricula_model->rpm_usuario_apruebareprueba = $usu_id;
                 \app\models\Utilities::putMessageLogFile('>>>>>>>>>>>>>> APROBACION RCHAZO DE PAGO DE INSCRIPCION ');
                 \app\models\Utilities::putMessageLogFile('Id de pago matricula: '.$id);
                  \app\models\Utilities::putMessageLogFile('Estado de pago matricula: '.$state);
                 \app\models\Utilities::putMessageLogFile('>>>>>>>>>>>>>> APROBACION RCHAZO DE PAGO DE INSCRIPCION ');
                if ($registro_pago_matricula_model->save()) {

                    //Ini analistadesarrollo03  detalle de factura pendien incluir que sea credito directo
                     $datos = $mod_pagos->consultarDetallePAgoFacturaByRpm($id);
                     \app\models\Utilities::putMessageLogFile('analistadesarrollo03. 1.0 dpfa_id:'.$datos['dpfa_id']);
                     $dpfa_id = $datos['dpfa_id'];
                     $fpe_id = $datos['fpe_id'];
                     $dpfa_valor = $datos['dpfa_valor'];
                     $observacion = "";
                     //actualiza estado de factura detalle
                    if($dpfa_id> 0){
                        if($state==2){
                            \app\models\Utilities::putMessageLogFile('>>>>>>>>>>>>>> RECHAZO: dpfactura: '.$dpfa_id);
                            \app\models\Utilities::putMessageLogFile('>>>>>>>>>>>>>> RECHAZO: Cuota  facetura pendiente fpe_id : '.$fpe_id);
                            \app\models\Utilities::putMessageLogFile('>>>>>>>>>>>>>> RECHAZO: Cuota  facetura pendiente  dpfa_valor: '.$dpfa_valor);
                             $respago = $mod_pagos->grabarRechazo($dpfa_id, 3, $observacion); //rechazo 
                             //actualizar saldos   detalle de pago que realizo
                            $updatePend =  $mod_facturas_pendientes_est->updateSaldosRechazoPagoInscripcion($fpe_id,$dpfa_valor,$usu_id);
                        }
                         else       
                            $respago = $mod_pagos->grabarRechazo($dpfa_id, 2, $observacion); //Aprobación
                    }
                     
                     //fin analistadesarrollo03
    ...
    public function actionExportoexcel() {
        ...
        $arrHeader = array(
          academico::t("matriculacion", "Date"),
          academico::t("Academico", "Student"),
          academico::t("matriculacion", "Passport"),
          academico::t("matriculacion", "Carrera"),
          academico::t("matriculacion", "Modalidad"),
          academico::t("Academico", "Periodo Academico"),
          academico::t("matriculacion", ""),
          academico::t("matriculacion", "Estado"),
        );
              $data = Yii::$app->request->get();
        $arrSearch["periodo"] = $data['periodo'];
        $arrData = array();
        $mod_periodo = new RegistroPagoMatricula();  
        $arrData = $mod_periodo->getAllRegistroMatriculaxGenerarGrid();
      $arrData =$mod_periodo->getAllRegistroMatriculaxGenerarGrid($data["search"], $data["modalidad"], $data["estado"], $data["carrera"], $data["periodo"],$data["fechaini"],$data["fechafin"], false);


        //$asignaturas_model = new AsignaturasPorPeriodo();
        //$arrData = array("C", "D", "E", "F", "G", "H", "I", "J", "K", "L", "M", "N");
        //if (empty($arrSearch)) {
       ...
       // }
        $nameReport = academico::t("Academico", "Estudiantes Inscritos");
