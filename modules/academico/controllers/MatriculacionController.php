				...
				if (($periodo_id != 0) and ($par_id != 0)) {
					...
					if (!$resp_consMatricula) {
						...
						if ($resp_planificacion) {
							//Si existe código de planificación
...
//presencial
	...
	public function actionList() {
		// listar estudiantes por aprobar matriculacion
	...
	public function actionRegistry($id) {
		// pantalla para aprobar matriculacion de estudiante
...
$dataValue[] = ['Id' => $con, 'Pago' => Academico::t('matriculacion',"Payment") . ' #' . $con, 'Vencimiento' => $ven, 'Porcentaje' => $value->roc_porcentaje, 'Valor' => '$'.(number_format($cost, 2, '.', ','))];
	...
	public function actionView($id) {
		// pantalla para ver la aprobacion de la matriculacion de un estudiante
		$model = RegistroOnline::findOne($id);
		if ($model) {
			$data = Yii::$app->request->get();
			$rama_id = $data['rama_id'];
	...
	public function actionUpdatepagoregistro() {
		// ACCION PARA SUBIR PAGO MATRICULA
		$data = Yii::$app->request->get();
		if ($data['filename']) {
	 ...
	 * @return
	 */
	public function actionRegistro($uper_id = Null) {
		// pantalla para que el estudiante seleccione las materias a registrarse
				...
				if (isset($data["pes_id"])) {
					...
					if ($id > 0) {
						...
						if ($RegistroOnline['ron_valor_gastos_adm'] <= 0) {
							...
							if ($block_roi == $bloque) {
								// Si uno de ellos es diferente, quiere decir que hay más de un bloque
	...
	public function actionRegistrodetalle($uper_id = Null) {
		...
		if ($periodo['saca_intensivo']) {
			...
			foreach ($tempBlock as $key => $value) {
		...
		try {
			...
			if (count($result_process) > 0) {
				...
				if (isset($data['cancelSubject']) && $data['cancelSubject'] = '1') {
					...
					if ($modelRegPag) {
// existe un pago de registro por lo que debe realizar el proceso
					...
					if ($i == $subReg) {
						// Se debe continuar con la cancelacion normal
						$template = "cancelregister";
...
*/
						...
						// como no es igual entonces se hace eliminacion de la materias seleccionadas
						$modelRegItem = RegistroOnlineItem::findAll(['ron_id' => $ron_id]);
						$sumMatr = 0;
						foreach ($modelRegItem as $key => $item) {
