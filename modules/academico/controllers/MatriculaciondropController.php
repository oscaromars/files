...
class MatriculaciondropController extends \app\components\CController {
	...
	public function actionUpdatepagoregistro() {
		// accion para aprobar matriculacion de estudiante
		$data = Yii::$app->request->get();
		$today = date("Y-m-d H:i:s");
		...
		if (Yii::$app->request->isAjax) {
			...
			try {
				...
				if ($data["estado"] == 1) {
					$model_registro_pago_matricula->rpm_estado_generado = '2';
				}
				// si esta aprobado /**0->por pagar, 1->por revisar, 2->pagado */
				else {
					$model_registro_pago_matricula->rpm_estado_generado = '0';
				}
				// se coloca nuevamente estado de por pagar
