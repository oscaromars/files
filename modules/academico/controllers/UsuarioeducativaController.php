		...
		if (Yii::$app->request->isAjax) {
			...
			try {
				if (!empty($asignado)) {
					...
					foreach ($asignados as $est_id) {
						// empieza foreach para guardar los asignados
		...
		if (Yii::$app->request->isAjax) {
			...
			try {
				if (!empty($nobloqueado)) {
					...
					foreach ($nobloqueado as $ceest_id) {
						// empieza foreach para guardar los asignados
			...
			*/
			...
			try {
				...
				for ($i = 0; $i < sizeof($data["nombre"]); $i++) {
					...
					$asignatura_paralelo = $data["nombre"][$i]["asignatura"]; //Dato del registro del grid
					//\app\models\Utilities::putMessageLogFile('Asignatura: '. $asignatura_paralelo);
					...
					//\app\models\Utilities::putMessageLogFile('mpp_id '. $mpp_id);
					//obtengo daca_id
					$daca_id = $distributivo_model->consultarDistribuAca($paca_id, $asi_id1, $mpp_id, $pro_id, $uaca_id, $mod_id);
