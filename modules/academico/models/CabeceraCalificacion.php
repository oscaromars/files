...
class CabeceraCalificacion extends \yii\db\ActiveRecord
{
    ...
    */
    ...
    public function getSupletorioPorIDs($est_id, $asi_id, $pro_id, $paca_id){
        ...
        $sql = "SELECT * FROM db_academico_mbtu.cabecera_calificacion as ccal
    ...
    public function actualizarCabeceraCalificacion($ccal_id, $cal_calif){
        ...
        try {
            $sql = "UPDATE db_academico_mbtu.cabecera_calificacion SET ccal_calificacion = $cal_calif WHERE ccal_id = $ccal_id";
    ...
    public function actualizarDetalleCalificacionporcomponente($ccal_id,$key , $value){
        ...
        $sql = "UPDATE db_academico_mbtu.detalle_calificacion
                   SET dcal_calificacion = $value
                 WHERE dcal_id = (
                       SELECT valor FROM(
                              SELECT dc1.dcal_id as valor
                                from db_academico_mbtu.detalle_calificacion dc1,
                                     db_academico_mbtu.componente_unidad cu1,
                                     db_academico_mbtu.componente com1
    ...
    public function crearDetalleCalificacionporcomponente($ccal_id,$key,$value,$uaca_id){
        ...
        if($value == ''){
            ...
            //$com_nombre = array_key_first($value);    
            $sql = "INSERT INTO `db_academico_mbtu`.`detalle_calificacion`
                                (
                                   `ccal_id`,
                                   `cuni_id`,
                         ...
                         VALUES (
                                    $ccal_id,
                                    (SELECT cuni.cuni_id
                                       from db_academico_mbtu.componente_unidad cuni,
                                            db_academico_mbtu.componente com
    ...
    public function actualizarDetalleCalificacion($ccal_id, $cuni_id, $dcal_calificacion){
        ...
        try {
            $sql = "UPDATE db_academico_mbtu.detalle_calificacion SET dcal_calificacion = $dcal_calificacion WHERE ccal_id = $ccal_id AND cuni_id = $cuni_id";
    ...
    public function actualizarDetalleCalificacion2($ccal_id, $dcal_calificacion){
        ...
        try {
            $sql = "UPDATE db_academico_mbtu.cabecera_calificacion 
     ...
     *  Consulta dal calificaciones de los estudiantes pot Docente y Priodo academico y  asignatura
     */
    public function consultaCalificacionRegistroDocenteSearch($uaca_id,$paca_id,$asi_id,$pro_id,$par_id, $onlyData = false){
        $con = \Yii::$app->db_academico;
        $con1 = \Yii::$app->db_asgard;;
        $estado = 1;
        ...
        if ($par_id != "" && $par_id > 0) {
            $str_search .= " paralelo.par_id =  :par_id AND ";
        }   
        ...
        $sql = "
            SELECT 
                ...
                resultado.* FROM (
                        SELECT  
                        ( SELECT g.gest_descripcion as value
                            FROM " . $con->dbname . ".grupo_estacion g
                            inner join " . $con->dbname . ".periodo_academico p ON g.gest_id = p.gest_id
                            WHERE  paca_id = :paca_id AND
                            p.paca_estado = '1' AND
                            p.paca_estado_logico = '1' AND
                            g.gest_estado = '1' AND
                            g.gest_estado_logico = '1')  as paca_nombre,
                        ccal_id,
                        est.est_id,
                        est.est_matricula,
                        concat(per.per_pri_nombre,' ',per.per_pri_apellido) as Nombres_completos,
                        ...
                        (SELECT clfc3.ccal_calificacion FROM " . $con->dbname . ".cabecera_calificacion clfc3
                                ...
                                INNER JOIN " . $con->dbname . ".esquema_calificacion esquema_calificacion3 ON esquema_calificacion3.ecal_id = ecun_par3.ecal_id
                                    WHERE   clfc3.est_id = clfc.est_id AND
                                            ...
                                            ecun_par3.uaca_id = :uaca_id  ),'NN')  as 'supletorio',
                        (SELECT clfc1.ccal_calificacion FROM " . $con->dbname . ".cabecera_calificacion clfc1
                                INNER JOIN " . $con->dbname . ".esquema_calificacion_unidad ecun_par ON ecun_par.ecun_id = clfc1.ecun_id
                                INNER JOIN " . $con->dbname . ".esquema_calificacion esquema_calificacion1 ON esquema_calificacion1.ecal_id = ecun_par.ecal_id
                                    WHERE   clfc1.est_id = est.est_id AND
                                            ...
                                            ecun_par.uaca_id = :uaca_id  ) as 'Parcial_I',
                        (SELECT clfc2.ccal_calificacion FROM ". $con->dbname . ".cabecera_calificacion clfc2
                        ...
                        (SELECT  casi.casi_porc_total
                         FROM db_academico_mbtu.cabecera_asistencia casi
                            INNER JOIN db_academico_mbtu.asistencia_esquema_unidad aeun_id_asistencia ON aeun_id_asistencia.aeun_id = casi.aeun_id
                            INNER JOIN db_academico_mbtu.esquema_calificacion_unidad esquema_calificacion_unidad ON esquema_calificacion_unidad.ecun_id = aeun_id_asistencia.aeun_id
                            INNER JOIN db_academico_mbtu.esquema_calificacion esquema_calificacion_asistencia ON  esquema_calificacion_asistencia.ecal_id = esquema_calificacion_unidad.ecal_id
                    ...
                    FROM  " . $con->dbname . ".estudiante est
                        ...
                        INNER JOIN " . $con->dbname . ".distributivo_academico_estudiante daca_est ON daca_est.mppd_id = daca.mppd_id
                        INNER JOIN " . $con->dbname . ".materias_paralelos_periodo_detalle mppd ON mppd.mppd_id =  daca_est.mppd_id
                        INNER JOIN " . $con->dbname . ".paralelo paralelo ON  paralelo.par_id = mppd.par_id
                        INNER JOIN " . $con->dbname . ".materias_paralelos_periodo materias_para_per ON  materias_para_per.mppe_id = mppd.mppe_id
                        INNER JOIN " . $con->dbname . ".asignaturas_por_periodo asi_per ON asi_per.aspe_id = materias_para_per.aspe_id
                        INNER JOIN " . $con->dbname . ".asignatura  asignatura2 ON  asignatura2.asi_codigo = asi_per.aspe_codigo
                    ...
                    WHERE   
                        ...
                        AND asi_per.paca_id = clfc.paca_id
                    ...
                    /*GROUP BY  est.est_id ,paralelo.par_nombre*/
                ) AS resultado";
    ...
    public function consultaCalificacionRegistroDocenteAllSearch($uaca_id,$paca_id,$asi_id,$pro_id,$par_id, $onlyData = false){
        ...
        $sql = "
                ...
                IFNULL(
                (SELECT 
                    clfc3.ccal_calificacion 
                    FROM " . $con->dbname . ".cabecera_calificacion clfc3
                        INNER JOIN " . $con->dbname . ".esquema_calificacion_unidad ecun_par3 ON ecun_par3.ecun_id = clfc3.ecun_id
                        INNER JOIN " . $con->dbname . ".esquema_calificacion esquema_calificacion3 ON esquema_calificacion3.ecal_id = ecun_par3.ecal_id
                    WHERE clfc3.est_id = clfc.est_id AND
                    clfc3.pro_id = clfc.pro_id AND
                    ...
                    ecun_par3.uaca_id = ecun.uaca_id ),'NN') as 'supletorio',
                (SELECT clfc1.ccal_calificacion 
                    FROM " . $con->dbname . ".cabecera_calificacion clfc1
                        INNER JOIN " . $con->dbname . ".esquema_calificacion_unidad ecun_par ON ecun_par.ecun_id = clfc1.ecun_id
                        INNER JOIN " . $con->dbname . ".esquema_calificacion esquema_calificacion1 ON esquema_calificacion1.ecal_id = ecun_par.ecal_id
                    WHERE clfc1.est_id = clfc.est_id AND
                        ...
                        ecun_par.uaca_id = ecun.uaca_id ) as 'Parcial_I',

                (SELECT clfc2.ccal_calificacion 
                    FROM " . $con->dbname . ".cabecera_calificacion clfc2
                        INNER JOIN " . $con->dbname . ".esquema_calificacion_unidad ecun_par2 ON ecun_par2.ecun_id = clfc2.ecun_id
                        INNER JOIN " . $con->dbname . ".esquema_calificacion esquema_calificacion2 ON esquema_calificacion2.ecal_id = ecun_par2.ecal_id
                    WHERE

                        clfc2.est_id = clfc.est_id AND
                        clfc2.pro_id = clfc.pro_id AND
                        clfc2.asi_id = clfc.asi_id AND
                 ...
                 (SELECT  casi.casi_porc_total
                         FROM " . $con->dbname . ".cabecera_asistencia casi
                            ...
                            INNER JOIN " . $con->dbname . ".esquema_calificacion_unidad esquema_calificacion_unidad ON esquema_calificacion_unidad.ecun_id = aeun_id_asistencia.aeun_id
                ...
                asignatura2.asi_descripcion as asi_nombre
                    -- sum(ccal.ccal_calificacion)
                FROM db_academico_mbtu.estudiante estudiante

                LEFT JOIN " . $con->dbname . ".cabecera_calificacion clfc ON estudiante.est_id =  clfc.est_id 
                LEFT JOIN " . $con->dbname . ".esquema_calificacion_unidad ecun ON ecun.ecun_id = clfc.ecun_id
                INNER JOIN " . $con1->dbname . ".persona  persona ON persona.per_id = estudiante.per_id 
                INNER JOIN " . $con->dbname . ".distributivo_academico_estudiante daca_est ON daca_est.est_id = estudiante.est_id
                INNER JOIN " . $con->dbname . ".materias_paralelos_periodo_detalle mppd ON mppd.mppd_id = daca_est.mppd_id
                INNER JOIN " . $con->dbname . ".paralelo paralelo ON paralelo.par_id = mppd.par_id
                INNER JOIN " . $con->dbname . ".materias_paralelos_periodo materias_para_per ON materias_para_per.mppe_id = mppd.mppe_id
                INNER JOIN " . $con->dbname . ".asignaturas_por_periodo asi_per ON asi_per.aspe_id = materias_para_per.aspe_id
                INNER JOIN " . $con->dbname . ".asignatura asignatura2 ON asignatura2.asi_codigo = asi_per.aspe_codigo
                INNER JOIN " . $con->dbname . ".distributivo_academico daca on  daca_est.mppd_id = daca.mppd_id
                 where 
                    ...
                    and mppd.par_id = :par_id
                    AND daca.paca_id = :paca_id
                    and daca.pro_id = :pro_id*/

                     AND (clfc.asi_id = daca.asi_id OR clfc.asi_id IS NULL)
                     AND (clfc.pro_id = daca.pro_id or clfc.pro_id  is null)
                     AND (clfc.paca_id = daca.paca_id OR clfc.paca_id IS NULL) 
                    ...
                    AND asignatura2.asi_estado = 1 
                    AND asignatura2.asi_estado_logico = 1
                    and (clfc.ccal_estado = 1 OR clfc.ccal_estado IS NULL)
                    and (clfc.ccal_estado_logico = 1 OR clfc.ccal_estado_logico IS NULL)
                ...
                FROM (
                    SELECT 
                        ...
                        IFNULL((SELECT  clfc3.ccal_calificacion 
                                FROM db_academico_mbtu.cabecera_calificacion clfc3
                                INNER JOIN db_academico_mbtu.esquema_calificacion_unidad ecun_par3 ON ecun_par3.ecun_id = clfc3.ecun_id
                                INNER JOIN db_academico_mbtu.esquema_calificacion esquema_calificacion3 ON esquema_calificacion3.ecal_id = ecun_par3.ecal_id
                                WHERE clfc3.est_id = clfc.est_id AND
                                clfc3.pro_id = clfc.pro_id AND
                                clfc3.asi_id = clfc.asi_id AND
                                clfc3.ccal_estado = 1 AND
                        ...
                        (   SELECT clfc1.ccal_calificacion 
                            FROM db_academico_mbtu.cabecera_calificacion clfc1
                            INNER JOIN db_academico_mbtu.esquema_calificacion_unidad ecun_par ON ecun_par.ecun_id = clfc1.ecun_id
                            INNER JOIN db_academico_mbtu.esquema_calificacion esquema_calificacion1 ON esquema_calificacion1.ecal_id = ecun_par.ecal_id
                            WHERE clfc1.est_id = clfc.est_id AND
                            clfc1.pro_id = clfc.pro_id AND
                            clfc1.asi_id = clfc.asi_id AND
                            clfc1.ccal_estado = 1 AND
                        ...
                        (   SELECT clfc2.ccal_calificacion 
                            FROM db_academico_mbtu.cabecera_calificacion clfc2
                            INNER JOIN db_academico_mbtu.esquema_calificacion_unidad ecun_par2 ON ecun_par2.ecun_id = clfc2.ecun_id
                            INNER JOIN db_academico_mbtu.esquema_calificacion esquema_calificacion2 ON esquema_calificacion2.ecal_id = ecun_par2.ecal_id
                    ...
                    FROM db_academico_mbtu.estudiante estudiante
                    LEFT JOIN db_academico_mbtu.cabecera_calificacion clfc ON estudiante.est_id =  clfc.est_id 
                    LEFT JOIN db_academico_mbtu.esquema_calificacion_unidad ecun ON ecun.ecun_id = clfc.ecun_id
                    INNER JOIN db_asgard_mbtu.persona persona ON persona.per_id = estudiante.per_id 
                    LEFT JOIN db_academico_mbtu.distributivo_academico_estudiante daca_est ON daca_est.est_id = estudiante.est_id
                    LEFT JOIN db_academico_mbtu.materias_paralelos_periodo_detalle mppd ON mppd.mppd_id = daca_est.mppd_id
                    LEFT JOIN db_academico_mbtu.paralelo paralelo ON paralelo.par_id = mppd.par_id
                    LEFT JOIN db_academico_mbtu.materias_paralelos_periodo materias_para_per ON materias_para_per.mppe_id = mppd.mppe_id
                    LEFT JOIN db_academico_mbtu.asignaturas_por_periodo asi_per ON asi_per.aspe_id = materias_para_per.aspe_id
                    LEFT JOIN db_academico_mbtu.asignatura asignatura2 ON asignatura2.asi_codigo = asi_per.aspe_codigo
                    INNER JOIN db_academico_mbtu.estudiante_carrera_programa AS ecpr ON ecpr.est_id = estudiante.est_id
                    INNER JOIN db_academico_mbtu.modalidad_estudio_unidad AS meun ON meun.meun_id = ecpr.meun_id
                    INNER JOIN db_academico_mbtu.periodo_academico AS paca ON paca.paca_id = asi_per.paca_id
                    INNER JOIN db_academico_mbtu.grupo_estacion AS gest ON gest.gest_id = paca.gest_id
                    WHERE 
                    $str_search 
                    $str_perfil_user
                    estudiante.est_activo = 1
