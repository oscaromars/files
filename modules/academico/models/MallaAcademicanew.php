...
class MallaAcademica extends \yii\db\ActiveRecord
{
    ...
    public function consultarasignaturaxmallaaut($per_id) {
        ...
        left join " . $con->dbname . ".planificacion_estudiante ples on made.made_codigo_asignatura in 
( ples.pes_mat_b1_h1_cod,ples.pes_mat_b1_h2_cod,ples.pes_mat_b1_h3_cod,ples.pes_mat_b1_h4_cod,ples.pes_mat_b1_h5_cod,ples.pes_mat_b1_h6_cod,
  ples.pes_mat_b2_h1_cod,ples.pes_mat_b2_h2_cod,ples.pes_mat_b2_h3_cod,ples.pes_mat_b2_h4_cod,ples.pes_mat_b2_h5_cod,ples.pes_mat_b2_h6_cod)
            where maes.per_id = $per_id and pmac.enac_id not in (1,4)
            and made.made_codigo_asignatura not in (
                SELECT made.made_codigo_asignatura
                from " . $con->dbname . ".malla_academico_estudiante maes
                inner join " . $con->dbname . ".promedio_malla_academico pmac on pmac.maes_id = maes.maes_id
                inner join " . $con->dbname . ".asignatura a on a.asi_id = maes.asi_id
                inner join " . $con->dbname . ".malla_academica_detalle made on made.made_id = maes.made_id,
                ...
                where maes.per_id = $per_id and pmac.enac_id not in (1,4)
                and trim(made.made_codigo_asignatura) in
                ( ples.pes_mat_b1_h1_cod,ples.pes_mat_b1_h2_cod,ples.pes_mat_b1_h3_cod,ples.pes_mat_b1_h4_cod,ples.pes_mat_b1_h5_cod,ples.pes_mat_b1_h6_cod,
                ples.pes_mat_b2_h1_cod,ples.pes_mat_b2_h2_cod,ples.pes_mat_b2_h3_cod,ples.pes_mat_b2_h4_cod,ples.pes_mat_b2_h5_cod,ples.pes_mat_b2_h6_cod))";    
...
inner join db_academico.modalidad_estudio_unidad c on c.meun_id = b.meun_id
 ...
 inner join db_academico.promedio_malla_academico b on a.maes_id = b.maes_id
   ...
   and a.asi_id = " . $requisito . "
                ...
                elseif ($i == 5) 
                    $asi6 = $rows_in[$i]["made_codigo_asignatura"]; 
                     $noasi6 = $rows_in[$i]["asi_nombre"];
                      ...
                      where mod_id = " . $mod_id . " 
                      and pla_estado = 1 and pla_estado_logico = 1 and pla_periodo_academico = " . $estacion;  
                ...
                $sql = "select pla_id from db_academico.planificacion where mod_id = " . $mod_id . " and pla_estado = 1 and pla_estado_logico = 1 and pla_periodo_academico = " . $estacion;
                  $comando = $con->createCommand($sql);
                   $rows_pla = $comando->queryOne();
            } 
