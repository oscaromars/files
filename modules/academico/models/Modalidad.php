...
class Modalidad extends \app\modules\academico\components\CActiveRecord {
	...
	public static function consultarIdsModalidad($TextAlias) {
		...
		//en caso de que existe problema o no retorne nada tiene 1 por defecto
		return $rawData;
	}
	 ...
	 * @return
	 */
	public function consultarModalidadxEstudiante($uaca_id, $est_id) {
		// \app\models\Utilities::putMessageLogFile('unidad academica modelo '. $uaca_id);
		$con = \Yii::$app->db_academico;
		$estado = 1;
		$sql = "SELECT distinct moda.mod_id as id,
                    ...
                    inner join " . $con->dbname . ".estudiante_carrera_programa ecpr on est.est_id = ecpr.est_id
                    inner join " . $con->dbname . ".modalidad_estudio_unidad meu on meu.meun_id=ecpr.meun_id
                    INNER JOIN " . $con->dbname . ".modalidad moda ON moda.mod_id = meu.mod_id
                    WHERE uaca_id = :uaca_id
                    and est.est_id = :est_id
                    and est.est_estado = :estado and est.est_estado_logico = :estado
                    and meu.meun_estado = :estado and meu.meun_estado_logico = :estado
