...
inner join db_academico.materia_paralelo_periodo mpp on mpp.asi_id = made.asi_id and mpp.paca_id in (28,29) and mpp.mod_id = :mod_id
 ...
 '2021-08-30 17:10:53', '1')
   ...
   $getallbymod = "
     ...
     * @return  
     */
    public function generateDatatotrasferOriginal($pla_id) {
    $con = \Yii::$app->db_academico;        
    $getallbymod = "
        SELECT
...
public function cerrarPlanificacionaut($pla_id) {
    ...
    public function consultarDatoscabplanifica($mod_id, $pla_periodo_academico) {
        ...
        \app\models\Utilities::putMessageLogFile('consultarDatoscabplanifica: '.$comando->getRawSql());
        $resultData = $comando->queryOne();
        return $resultData;
    }
    ...
    public function busquedaEstudianteasis($per_id) {
        $con = \Yii::$app->db_academico;
        $estado = 1;
     ...
     */
    /* MODIFICAR DATOS */
    public function modificarDataPlanificacionestudiante($pla_id, $per_id, $pes_cod_carrera, $pes_usuario_modifica, $modificar) {
        $arroout = array();
        $con = \Yii::$app->db_academico;
        $trans = $con->beginTransaction();
        ...
        try {
            $data = isset($data['DATA']) ? $data['DATAS'] : array();
            $this->modificarPlanificacionestudiante($con,$pla_id, $per_id, $pes_cod_carrera, $pes_usuario_modifica,$fecha, $modificar);
            $trans->commit();
            ...
            \app\models\Utilities::putMessageLogFile('modificarDataPlanificacionestudiante: '.$e->getMessage());
     ...
     * @return
     */
    public function modificarPlanificacionestudiante($con,$pla_id, $per_id, $pes_cod_carrera, $pes_usuario_modifica,$fecha, $modificar) {
        ...
        try {
            ...
            \app\models\Utilities::putMessageLogFile('pes_cod_carrera comando: '.$pes_cod_carrera);
            $comando = $con->createCommand
                    ("UPDATE " . $con->dbname . ".planificacion_estudiante      
                      ...
                      WHERE
                        pes_cod_carrera = '" . $pes_cod_carrera . "' AND
                        pla_id = " . $pla_id . " AND                        
                        per_id = " . $per_id . " AND
                        pes_estado = :estado AND
                        pes_estado_logico = :estado");
            ...
            $comando->bindParam(":estado", $estado, \PDO::PARAM_STR);
            \app\models\Utilities::putMessageLogFile('modificarPlanificacionestudiante: '.$comando->getRawSql());
            ...
            \app\models\Utilities::putMessageLogFile('modificarPlanificacionestudiante: '.$ex->getMessage());
            if ($trans !== null)
                $trans->rollback();
            return FALSE;
        }
    ...
    public function agregarMateriasPlanificacionEstudiante($pla_id, $per_id) {
        ...
        try {
            ...
            \app\models\Utilities::putMessageLogFile('agregarMateriasPlanificacionEstudiante: '.$ex->getMessage());
...
AND pes.pla_id = :pla_id ) as daho12                    
    ...
    public function getPlanificacionxPeriodo($paca_id,$per_id){
        ...
        public function consultarEstxMatPlan($arrFiltro = array(),$onlyData = false) {
            ...
            $resultData = $comando->queryAll();
            \app\models\Utilities::putMessageLogFile('consultarEstxMatPlan: '.$comando->getRawSql());
            //////\app\modules\academico\controllers\RegistroController::putMessageLogFileCartera('consultarModalidad: '.$comando->getRawSql());
            $dataProvider = new ArrayDataProvider([
                'key' => 'id',
                'allModels' => $resultData,
        ...
        public function modificarPlanificacionestudianteContingencia($pla_id, $per_id, $pes_usuario_modifica, $modificar) {
            $con = \Yii::$app->db_academico;
            $fecha = date(Yii::$app->params["dateTimeByDefault"]);
            $estado = 1;
            $trans = $con->getTransaction(); // se obtiene la transacción actual
            ...
            try {
                ...
                \app\models\Utilities::putMessageLogFile('modificarPlanificacionestudianteContingencia ');
                $comando = $con->createCommand
                        ("UPDATE " . $con->dbname . ".planificacion_estudiante      
                          SET pes_usuario_modifica = " . $pes_usuario_modifica . ", " . 
                          $modificar . "
                ...
                $comando->bindParam(":estado", $estado, \PDO::PARAM_STR);
                \app\models\Utilities::putMessageLogFile('modificarPlanificacionestudianteContingencia: '.$comando->getRawSql());
                ...
                \app\models\Utilities::putMessageLogFile('modificarPlanificacionestudianteContingencia: '.$ex->getMessage());
                if ($trans !== null)
                    $trans->rollback();
                return FALSE;
            }
