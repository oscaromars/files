...
function cargarDocumento() {
    var arrParams = new Object();
    var link = $('#txth_base').val() + "/documental/gestion/upload";
    arrParams.procesar_file = true;
    arrParams.archivo = $('#txth_doc_adj_documento2').val() + "." + $('#txth_doc_adj_documento').val().split('.').pop();
    if (!validateForm()) {
        requestHttpAjax(link, arrParams, function (response) {
            ...
            if (!response.error) {
                setTimeout(function () {
                    window.location.href = $('#txth_base').val() + "/documental/gestion/index";
                }, 3000);
            }
        }, true);
    }
