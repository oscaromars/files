 ...
 */
function edit() {
    var link = $('#txth_base').val() + "/" + $('#txth_module').val() + "/discapacidad/edit" + "?id=" + $("#frm_id").val();
 ...
 */
function update() {
    var link = $('#txth_base').val() + "/" + $('#txth_module').val() + "/discapacidad/update";
    var arrParams = new Object();
    ...
    if (!validateForm()) {
        requestHttpAjax(link, arrParams, function(response) {
            ...
            if (response.status == "OK") {
                setTimeout(function() {
                    window.location.href = $('#txth_base').val() + "/" + $('#txth_module').val() + "/discapacidad/index";
                }, 3000);
            }
        }, true);
    }
...
function deleteItem(id, cod) {
    var link = $('#txth_base').val() + "/" + $('#txth_module').val() + "/discapacidad/delete";
    var arrParams = new Object();
    arrParams.id = id;
    requestHttpAjax(link, arrParams, function(response) {
        if (response.status == "OK") {
...
function exportExcel() {
    ...
    window.location.href = $('#txth_base').val() + "/" + $('#txth_module').val() + "/discapacidad/expexcel?search=" + search;
}
...
function exportPdf() {
    var search = $('#txt_search').val();
    window.location.href = $('#txth_base').val() + "/" + $('#txth_module').val() + "/discapacidad/exppdf?pdf=1&search=" + search;
}