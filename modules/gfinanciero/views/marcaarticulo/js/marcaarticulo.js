 ...
 */
function edit() {
    var link = $('#txth_base').val() + "/" + $('#txth_module').val() + "/marcaarticulo/edit" + "?id=" + $("#frm_id").val();
 ...
 */
function update() {
    var link = $('#txth_base').val() + "/" + $('#txth_module').val() + "/marcaarticulo/update";
    var arrParams = new Object();
 ...
 */
function save() {
    var link = $('#txth_base').val() + "/" + $('#txth_module').val() + "/marcaarticulo/save";
    var arrParams = new Object();
    arrParams.id = $('#frm_id').val();
    arrParams.nombre = $('#frm_name').val();
    arrParams.fecha = $('#frm_fecha').val();
    if (!validateForm()) {
        requestHttpAjax(link, arrParams, function(response) {
            ...
            if (response.status == "OK") {
                setTimeout(function() {
                    window.location.href = $('#txth_base').val() + "/" + $('#txth_module').val() + "/marcaarticulo/index";
                }, 3000);
            }
        }, true);
    }
...
function deleteItem(id) {
    var link = $('#txth_base').val() + "/" + $('#txth_module').val() + "/marcaarticulo/delete";
    var arrParams = new Object();
    arrParams.id = id;
    requestHttpAjax(link, arrParams, function(response) {
        if (response.status == "OK") {
...
function exportExcel() {
    ...
    window.location.href = $('#txth_base').val() + "/" + $('#txth_module').val() + "/marcaarticulo/expexcel?search=" + search;
}
...
function exportPdf() {
    var search = $('#txt_search').val();
    window.location.href = $('#txth_base').val() + "/" + $('#txth_module').val() + "/marcaarticulo/exppdf?pdf=1&search=" + search;
}