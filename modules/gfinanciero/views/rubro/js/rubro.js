 ...
 */
function edit() {
    var link = $('#txth_base').val() + "/" + $('#txth_module').val() + "/rubro/edit" + "?id=" + $("#frm_id").val();
 ...
 */
function update() {
    var link = $('#txth_base').val() + "/" + $('#txth_module').val() + "/rubro/update";
    var arrParams = new Object();
    arrParams.id = $("#frm_id").val();
    arrParams.nombre = $('#frm_name').val();
    arrParams.type = $("#cmb_tipo").val();
    arrParams.cprincipal = $('#autocomplete-cuentaprincipal').val();
    arrParams.cprovisional = $('#autocomplete-cuentaprovisional').val();
    if (!validateForm()) {
 ...
 */
function save() {
    var link = $('#txth_base').val() + "/" + $('#txth_module').val() + "/rubro/save";
    var arrParams = new Object();
    arrParams.nombre = $('#frm_name').val();
    arrParams.type = $("#cmb_tipo").val();
    arrParams.cprincipal = $('#autocomplete-cuentaprincipal').val();
    ...
    if (!validateForm()) {
        requestHttpAjax(link, arrParams, function(response) {
            ...
            if (response.status == "OK") {
                setTimeout(function() {
                    window.location.href = $('#txth_base').val() + "/" + $('#txth_module').val() + "/rubro/index";
                }, 3000);
            }
        }, true);
    }
...
function deleteItem(id, cod) {
    var link = $('#txth_base').val() + "/" + $('#txth_module').val() + "/rubro/delete";
    var arrParams = new Object();
    arrParams.id = id;
    requestHttpAjax(link, arrParams, function(response) {
        if (response.status == "OK") {
...
function exportExcel() {
    ...
    window.location.href = $('#txth_base').val() + "/" + $('#txth_module').val() + "/rubro/expexcel?search=" + search + "&type=" + type;
}
...
function exportPdf() {
    var search = $('#txt_search').val();
    var type = $("#cmb_tipo").val();
    window.location.href = $('#txth_base').val() + "/" + $('#txth_module').val() + "/rubro/exppdf?pdf=1&search=" + search + "&type=" + type;
}
