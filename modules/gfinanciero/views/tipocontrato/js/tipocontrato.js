 ...
 */
function edit() {
    var link = $('#txth_base').val() + "/" + $('#txth_module').val() + "/tipocontrato/edit" + "?id=" + $("#frm_id").val();
 ...
 */
function update() {
    var link = $('#txth_base').val() + "/" + $('#txth_module').val() + "/tipocontrato/update";
    var arrParams = new Object();
 ...
 */
function save() {
    var link = $('#txth_base').val() + "/" + $('#txth_module').val() + "/tipocontrato/save";
    var arrParams = new Object();
    arrParams.nombre = $('#frm_name').val();
    ...
    if (!validateForm()) {
        requestHttpAjax(link, arrParams, function(response) {
            ...
            if (response.status == "OK") {
                setTimeout(function() {
                    window.location.href = $('#txth_base').val() + "/" + $('#txth_module').val() + "/tipocontrato/index";
                }, 3000);
            }
        }, true);
    }
...
function deleteItem(id, cod) {
    var link = $('#txth_base').val() + "/" + $('#txth_module').val() + "/tipocontrato/delete";
    var arrParams = new Object();
    arrParams.id = id;
    requestHttpAjax(link, arrParams, function(response) {
        if (response.status == "OK") {
...
function exportExcel() {
    ...
    window.location.href = $('#txth_base').val() + "/" + $('#txth_module').val() + "/tipocontrato/expexcel?search=" + search;
}
...
function exportPdf() {
    var search = $('#txt_search').val();
    window.location.href = $('#txth_base').val() + "/" + $('#txth_module').val() + "/tipocontrato/exppdf?pdf=1&search=" + search;
}