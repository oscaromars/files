 ...
 */
function edit() {
    var link = $('#txth_base').val() + "/" + $('#txth_module').val() + "/tipotransaccion/edit" + "?id=" + $("#frm_id").val();
 ...
 */
function update() {
    var link = $('#txth_base').val() + "/" + $('#txth_module').val() + "/tipotransaccion/update";
    var arrParams = new Object();
 ...
 */
function save() {
    var link = $('#txth_base').val() + "/" + $('#txth_module').val() + "/tipotransaccion/save";
    var arrParams = new Object();
    arrParams.id = $('#frm_id').val();
    arrParams.nombre = $('#frm_name').val();
    arrParams.type = $("#cmb_tipo").val();
    if (!validateForm()) {
        requestHttpAjax(link, arrParams, function(response) {
            ...
            if (response.status == "OK") {
                setTimeout(function() {
                    window.location.href = $('#txth_base').val() + "/" + $('#txth_module').val() + "/tipotransaccion/index";
                }, 3000);
            }
        }, true);
    }
...
function deleteItem(id) {
    var link = $('#txth_base').val() + "/" + $('#txth_module').val() + "/tipotransaccion/delete";
    var arrParams = new Object();
    arrParams.id = id;
    requestHttpAjax(link, arrParams, function(response) {
        if (response.status == "OK") {
...
function exportExcel() {
    ...
    window.location.href = $('#txth_base').val() + "/" + $('#txth_module').val() + "/tipotransaccion/expexcel?search=" + search + "&type=" + type;
}
...
function exportPdf() {
    var search = $('#txt_search').val();
    var type = $("#cmb_tipo").val();
    window.location.href = $('#txth_base').val() + "/" + $('#txth_module').val() + "/tipotransaccion/exppdf?pdf=1&search=" + search + "&type=" + type;
}