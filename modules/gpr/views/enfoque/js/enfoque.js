...
function edit() {
    var link = $('#txth_base').val() + "/gpr/enfoque/edit" + "?id=" + $("#frm_id").val();
    window.location = link;
}

function update() {
    var link = $('#txth_base').val() + "/gpr/enfoque/update";
    var arrParams = new Object();
...
function save() {
    var link = $('#txth_base').val() + "/gpr/enfoque/save";
    var arrParams = new Object();
    arrParams.nombre = $('#frm_name').val();
    arrParams.desc = $('#frm_desc').val();
    arrParams.estado = $('#frm_status').val();
    if (!validateForm()) {
        requestHttpAjax(link, arrParams, function(response) {
            ...
            if (response.status == "OK") {
                setTimeout(function() {
                    window.location.href = $('#txth_base').val() + "/gpr/enfoque/index";
                }, 3000);
            }
        }, true);
    }
...
function deleteItem(id) {
    var link = $('#txth_base').val() + "/gpr/enfoque/delete";
    var arrParams = new Object();
    arrParams.id = id;
    requestHttpAjax(link, arrParams, function(response) {
        if (response.status == "OK") {
