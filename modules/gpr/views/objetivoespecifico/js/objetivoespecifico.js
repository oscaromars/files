...
function searchModules(idbox, idgrid) {
    ...
    arrParams.unidad = $("#cmb_adm").val();
    $("#" + idgrid).PbGridView("applyFilterData", arrParams);
}

function edit() {
    var link = $('#txth_base').val() + "/gpr/objetivoespecifico/edit" + "?id=" + $("#frm_id").val();
    window.location = link;
}

function update() {
    var link = $('#txth_base').val() + "/gpr/objetivoespecifico/update";
    var arrParams = new Object();
    ...
    if ($('#cmb_obj').val() == 0) {
        var msg = objLang.Please_select_a_Strategic_Objective_;
        shortModal(msg, objLang.Error, "error");
        return;
    }
    if ($('#cmb_adm').val() == 0) {
        var msg = objLang.Please_select_an_Administrative_Unit_;
        shortModal(msg, objLang.Error, "error");
        return;
    }
    if (!validateForm()) {
...
function save() {
    var link = $('#txth_base').val() + "/gpr/objetivoespecifico/save";
    var arrParams = new Object();
    arrParams.nombre = $('#frm_name').val();
    arrParams.desc = $('#frm_desc').val();
    arrParams.objetivo = $('#cmb_obj').val();
    ...
    if (!validateForm()) {
        requestHttpAjax(link, arrParams, function(response) {
            ...
            if (response.status == "OK") {
                setTimeout(function() {
                    window.location.href = $('#txth_base').val() + "/gpr/objetivoespecifico/index";
                }, 3000);
            }
        }, true);
    }
...
function deleteItem(id) {
    var link = $('#txth_base').val() + "/gpr/objetivoespecifico/delete";
    var arrParams = new Object();
    arrParams.id = id;
    requestHttpAjax(link, arrParams, function(response) {
        if (response.status == "OK") {
