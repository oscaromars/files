...
function searchModules(idbox, idgrid) {
    ...
    arrParams.entidad = $("#cmb_plan").val();
    arrParams.cierre = $("#cmb_closed").val();
    $("#" + idgrid).PbGridView("applyFilterData", arrParams);
}

function edit() {
    var link = $('#txth_base').val() + "/gpr/planificacionpoa/edit" + "?id=" + $("#frm_id").val();
    window.location = link;
}

function update() {
    var link = $('#txth_base').val() + "/gpr/planificacionpoa/update";
    var arrParams = new Object();
...
function save() {
    var link = $('#txth_base').val() + "/gpr/planificacionpoa/save";
    var arrParams = new Object();
    arrParams.nombre = $('#frm_name').val();
    arrParams.desc = $('#frm_desc').val();
    arrParams.fini = $('#frm_fini').val();
    ...
    if (!validateForm()) {
        requestHttpAjax(link, arrParams, function(response) {
            ...
            if (response.status == "OK") {
                setTimeout(function() {
                    window.location.href = $('#txth_base').val() + "/gpr/planificacionpoa/index";
                }, 3000);
            }
        }, true);
    }
...
function deleteItem(id) {
    var link = $('#txth_base').val() + "/gpr/planificacionpoa/delete";
    var arrParams = new Object();
    arrParams.id = id;
    requestHttpAjax(link, arrParams, function(response) {
        if (response.status == "OK") {
