...
(function($) {
    ...
    $.fn.ajaxlivesearch = function(options) {
         ...
         */
        function search(search_object, form, options, bypass_check_last_value, reset_current_page, forceSearch) {
            forceSearch = forceSearch || false;
            var result = getFormInfo(form, 'result', options);

            if (($.trim(search_object.value).length && $.trim(search_object.value).length >= options.min_chars_to_search) || forceSearch == true) {
                /**
                 ...
                 */
                if (bypass_check_last_value || search_object.latest_value !== search_object.value || forceSearch == true) {
                    ...
                    search_object.to_be_executed = setTimeout(function() {
                        ...
                        if ($.trim(search_object.value).length || forceSearch == true) {
        ...
        this.each(function() {
            ...
            query.on('keyup', function(event) {
                // If enter key is pressed check if the user wants to select hovered row
                var keycode = event.keyCode || event.which;
                if ($.trim(query.val()).length && keycode === 13) { // 13 -> Enter
            ...
            query.on('keydown', function(event) {
                var keycode = event.keyCode || event.which;
                if (keycode === 40 || keycode === 38) { // 40 -> arrow down  38 -> arrow up
            ...
            arrow.on('click', function() {
                ...
                // search again
                search(query[0], form, ls, true, false, true);
            });
            ...
            page_range.on('change', function() {
                 ...
                 */
                search(query[0], form, ls, true, true, true);
            });
